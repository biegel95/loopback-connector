...
function(columnName, operator, columnValue, propertyValue) {
  function buildClause(columnValue, separator, grouping) {
    ...
    console.log("clause>grouping", clause.ParameterizedSQL)
    console.log("clause.ParameterizedSQL.params.length", clause.ParameterizedSQL.params.length)
    if (clause && clause.ParameterizedSQL && clause.ParameterizedSQL.params && clause.ParameterizedSQL.params.length > 0) {
      var newParams = '';
      clause.ParameterizedSQL.params.forEach(function(params, index) {
        if(index > 0) newParams = newParams + ' UNION ALL ';

        newParams = newParams + 'SELECT ' + params + ' FROM DUAL';
      });
      clause.ParameterizedSQL.params = newParams;
