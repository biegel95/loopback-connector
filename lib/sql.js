...
function(columnName, operator, columnValue, propertyValue) {
  function buildClause(columnValue, separator, grouping) {
    ...
    if (clause && clause.params && clause.params.length > 0) {
      var newParams = [];
      var newParamsString = '';

      clause.params.forEach(function(params, index) {
        if(index > 0) newParamsString = newParamsString + ' UNION ALL ';
        newParamsString = newParamsString + 'SELECT ' + params + ' FROM DUAL';
        //newParams.push(newParamsString);
        //newParamsString = '';
      });

      clause.params = [];
      clause.sql = '('+newParamsString+')'
      console.log("clause", clause);
    }
